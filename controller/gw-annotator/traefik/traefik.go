...
func (g *Gateway) IsAnnotated() bool {
	...
	case gateway.Ingress:
		headers, ok := gateway.GetCustomRequestHeaders(g.Object, HeadersSeparator)
		if ok {
...
func (g *Gateway) GenerateAnnotationPatch(clusterDomain string) (gateway.Patch, error) {
	...
	switch g.ConfigMode {
	case gateway.Ingress:
		headers, found := gateway.GetCustomRequestHeaders(g.Object, HeadersSeparator)
		op := "add"
		if found {
			op = "replace"
		}
		...
		return []gateway.PatchOperation{{
			Op:    op,
			Path:  gateway.AnnotationsPath + strings.Replace(gateway.CustomRequestHeadersKey, "/", "~1", -1),
